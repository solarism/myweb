...
- 直接點選圖中節點進入次領域，上方「返回主領域」可回到主圖。圖下方僅顯示領域文字，不設操作按鈕。方向鍵旋轉，加減鍵縮放，Home 恢復初始位置。
- 主圖與次領域圖以原始速度的 2 倍旋轉，每 8–16 秒平滑轉向新的隨機方向。滑鼠移入或鍵盤操作時暫停，離開後繼續，也可聚焦圖形本身後按空白鍵暫停或恢復自動旋轉。圖形離開畫面或分頁隱藏時停止動畫；偏好減少動態效果時預設關閉自動旋轉，仍可自行開啟或手動操作。
